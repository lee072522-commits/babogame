...
try {
    ...
    while ($listener.IsListening) {
        ...
        if ($urlPath -eq "/api/team-images") {
            ...
            $response.ContentLength64 = $bytes.Length
            $response.OutputStream.Write($bytes, 0, $bytes.Length)
        } elseif ($urlPath -eq "/api/special-images") {
            $imagesDir = Join-Path (Get-Location) "images"
            $json = "[]"
            if (Test-Path $imagesDir) {
                $files = Get-ChildItem -Path $imagesDir -Filter "special-*" | Where-Object { $_.Extension -match "jpg|jpeg|png" } | Select-Object -ExpandProperty Name
                if ($null -ne $files) {
                    if ($files -is [string]) {
                        $json = "[`"$files`"]"
                    } else {
            ...
            $response.ContentLength64 = $bytes.Length
            $response.OutputStream.Write($bytes, 0, $bytes.Length)
        } elseif ($urlPath -eq "/api/example-images") {
            $imagesDir = Join-Path (Get-Location) "images"
            $json = "[]"
            if (Test-Path $imagesDir) {
                $files = Get-ChildItem -Path $imagesDir -Filter "example-*" | Where-Object { $_.Extension -match "jpg|jpeg|png" } | Select-Object -ExpandProperty Name
                if ($null -ne $files) {
                    if ($files -is [string]) {
                        $json = "[`"$files`"]"
                    } else {
